...
TEST(SOSVM, sgd_check_w_helper)
{
	...
	auto instances = std::make_shared<CFactorGraphFeatures>(num_samples);

	auto labels = std::make_shared<CFactorGraphLabels>(num_samples);
	...
	for (int32_t n = 0; n < num_samples; ++n)
	{
		...
		var_index1[0] = 0;
		auto fac1 = std::make_shared<CFactor>(factortype, var_index1, data1);
		fg->add_factor(fac1);
	...
	auto model = std::make_shared<CFactorGraphModel>(instances, labels, TREE_MAX_PROD, false);
	...
	// SGD solver
	auto sgd = std::make_shared<CStochasticSOSVM>(model, labels, false, false);
	sgd->set_num_iter(1);
	sgd->set_lambda(1.0);
	sgd->train();
	w = sgd->get_w();
...
TEST(SOSVM, fw_check_w_helper)
{
	...
	auto fw = std::make_shared<CFWSOSVM>(model, labels, false, false);
	fw->set_num_iter(1);
	fw->set_lambda(1.0);
