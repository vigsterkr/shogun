...
TEST(TDistributedStochasticNeighborEmbeddingTest,basic)
{
	...
	auto high_dimensional_features =
		std::make_shared<CDenseFeatures<float64_t>>(CDataGenerator::generate_gaussians(n_samples, 1, n_dimensions));

	auto embedder =
		std::make_shared<CTDistributedStochasticNeighborEmbedding>();
