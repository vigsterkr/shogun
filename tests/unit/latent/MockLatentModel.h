...
namespace shogun {
	...
	class MockCLatentModel : public CLatentModel {
		public:
			...
			MOCK_METHOD0(get_psi_feature_vectors, std::shared_ptr<CDotFeatures>());
			MOCK_METHOD2(infer_latent_variable, std::shared_ptr<CData>(const SGVector<float64_t>& w, index_t idx));
