...
template <class T> class SGSparseMatrix : public SGReferencedData
{
	public:
		 ...
		 * @return label vector
		 */
		SGVector<float64_t> load_with_labels(std::shared_ptr<CLibSVMFile> libsvm_file, bool do_sort_features=true);

		/** save sparse matrix to file
		 *
		 * @param saver File object via which to save data
		 */
		void save(std::shared_ptr<CFile> saver);
		 ...
		 */
		void save_with_labels(std::shared_ptr<CLibSVMFile> saver, SGVector<float64_t> labels);
