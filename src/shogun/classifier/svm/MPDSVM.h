...
class CMPDSVM : public CSVM
{
	public:
		 ...
		 * @param lab labels
		 */
		CMPDSVM(float64_t C, std::shared_ptr<CKernel> k, std::shared_ptr<CLabels> lab);
		virtual ~CMPDSVM();
	...
	protected:
		...
		{
			return (std::static_pointer_cast<CBinaryLabels>(m_labels))->get_label(i)*
				(std::static_pointer_cast<CBinaryLabels>(m_labels))->get_label(j)*kernel->kernel(i,j);
		}
		...
		inline KERNELCACHE_ELEM* lock_kernel_row(int32_t i)
		{
			...
			if (!line)
			{
				...
				for (int32_t j=0; j<m_labels->get_num_labels(); j++)
					line[j]=(KERNELCACHE_ELEM) (std::static_pointer_cast<CBinaryLabels>(m_labels))->get_label(i)*(std::static_pointer_cast<CBinaryLabels>(m_labels))->get_label(j)*kernel->kernel(i,j);
			}
		...
		/** kernel cache */
		std::shared_ptr<CCache<KERNELCACHE_ELEM>> kernel_cache;
};
}
#endif  /* _MPDSVM_H___ */
