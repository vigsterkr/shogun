...
SGVector<int32_t> Statistics::sample_indices(int32_t sample_size, int32_t N)
{
...
{
	SG_SDEBUG("entering Statistics::fit_sigmoid()\n")
	...
	for (it=0; it<maxiter; ++it)
	{
		...
		if (stepsize<minstep)
		{
			SG_SWARNING("Statistics::fit_sigmoid(): line search fails, A=%f, "
					"B=%f, g1=%f, g2=%f, dA=%f, dB=%f, gd=%f\n",
					a, b, g1, g2, dA, dB, gd);
		}
	}
	...
	if (it>=maxiter-1)
	{
		SG_SWARNING("Statistics::fit_sigmoid(): reaching maximal iterations,"
				" g1=%f, g2=%f\n", g1, g2);
	}
	...
	SG_SDEBUG("leaving Statistics::fit_sigmoid()\n")
	return result;
}
