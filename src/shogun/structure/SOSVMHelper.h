...
public:
	 ...
	 */
	static float64_t primal_objective(SGVector<float64_t> w, std::shared_ptr<CStructuredModel> model, float64_t lbda);
	 ...
	 * @return average loss
	 */
	static float64_t average_loss(SGVector<float64_t> w, std::shared_ptr<CStructuredModel> model, bool is_ub = false);
