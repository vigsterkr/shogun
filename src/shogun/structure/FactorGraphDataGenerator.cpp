...
FactorGraphDataGenerator::FactorGraphDataGenerator(): SGObject()
{}
...
std::shared_ptr<FactorGraph> FactorGraphDataGenerator::random_chain_graph(SGVector<int> &assignment_expect, float64_t &min_energy_expect, int32_t N)
{
	...
	for (int32_t y = 0; y < N; ++y)
		for (int32_t x = 0; x < N; ++x)
		{
			...
			data[0] = Math::random(0.0, 1.0);
			data[1] = Math::random(0.0, 1.0);
	...
	for (int32_t x = 0; x < N; x++)
	{
		for (int32_t y = 0; y < N; y++)
		{
			if (x > 0)
			{
				...
				float64_t A = Math::random(0.0, 1.0);//E(0,0)->A
				float64_t C = Math::random(0.0, 1.0);//E(1,0)->C
				float64_t B = Math::random(0.0, 1.0);//E(0,1)->B
				float64_t D = Math::random(0.0, 1.0);//E(1,1)->D
