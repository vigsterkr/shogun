...
class CHMSVMModel : public CStructuredModel
{
	public:
		 ...
		 */
		CHMSVMModel(std::shared_ptr<CFeatures> features, std::shared_ptr<CStructuredLabels> labels, EStateModelType smt, int32_t num_obs=0, bool use_plifs=false);
		 ...
		 */
		std::shared_ptr<CStateModel> get_state_model() const;
	...
	private:
		...
		/** the state model */
		std::shared_ptr<CStateModel> m_state_model;
		...
		std::shared_ptr<CDynamicObjectArray> m_plif_matrix;
