...
public:
	 ...
	 */
	SGMatrix<float64_t> estimate_confusion_matrix(std::shared_ptr<CBaseMulticlassMachine >machine, std::shared_ptr<CFeatures >X, std::shared_ptr<CMulticlassLabels >Y, int32_t num_classes);
	 ...
	 */
	void get_confusion_matrix(SGMatrix<float64_t> &conf_mat, std::shared_ptr<CMulticlassLabels >gt, std::shared_ptr<CMulticlassLabels >pred);
};
