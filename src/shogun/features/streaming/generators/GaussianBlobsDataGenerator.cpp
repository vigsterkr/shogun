...
{
	SG_SDEBUG("entering GaussianBlobsDataGenerator::get_next_example()\n");

	/* allocate space */
	SGVector<float64_t> result=SGVector<float64_t>(2);

	/* sample latent distribution to compute offsets */
	index_t x_offset=Math::random(0, m_sqrt_num_blobs-1)*m_distance;
	index_t y_offset=Math::random(0, m_sqrt_num_blobs-1)*m_distance;

	/* sample from std Gaussian */
	float64_t x=Math::randn_double();
	float64_t y=Math::randn_double();
	...
	SG_SDEBUG("leaving GaussianBlobsDataGenerator::get_next_example()\n");
	return true;
}
