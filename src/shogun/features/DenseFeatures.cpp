		...
		int32_t vec_idx2)
{
	...
	SGVector<ST> vec1(get_feature_vector(vec_idx1, len1, free1), len1, false);
	SGVector<ST> vec2(sf->get_feature_vector(vec_idx2, len2, free2), len2, false);

	float64_t result = linalg::dot(vec1, vec2);
		...
		int32_t vec_idx1, const float64_t* vec2, int32_t vec2_len)
{
	...
	int32_t vlen;
	bool vfree;
	SGVector<float64_t> vec1(get_feature_vector(vec_idx1, vlen, vfree), vlen, false);

	ASSERT(vlen == num_features)
	SGVector<float64_t> tmp(const_cast<float64_t*>(vec2), vec2_len, false);
	float64_t result = linalg::dot(vec1, tmp);
