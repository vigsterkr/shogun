...
int32_t count_leaf_tasks_recursive(std::shared_ptr<CTask> subtree_root_block)
{
	...
	{
		int32_t sum = 0;
		auto iterator = sub_tasks->get_first_element()->as<CTask>();
		do
		{
			sum += count_leaf_tasks_recursive(iterator);
		}
		while ((iterator = sub_tasks->get_next_element()->as<CTask>()) != NULL);
...
void collect_tree_tasks_recursive(std::shared_ptr<CTask> subtree_root_block, vector<task_tree_node_t>* tree_nodes, int low)
{
	int32_t lower = low;
	auto sub_blocks = subtree_root_block->get_subtasks();
	if (sub_blocks->get_num_elements()>0)
	{
		auto iterator = sub_blocks->get_first_element()->as<CTask>();
		do
		{
		...
		while ((iterator = sub_blocks->get_next_element()->as<CTask>()) != NULL);
	}

}

void collect_leaf_tasks_recursive(std::shared_ptr<CTask> subtree_root_block, std::shared_ptr<CList> list)
{
	auto sub_blocks = subtree_root_block->get_subtasks();
	if (sub_blocks->get_num_elements() == 0)
	{
...
int32_t count_leaft_tasks_recursive(std::shared_ptr<CTask> subtree_root_block)
{
	auto sub_blocks = subtree_root_block->get_subtasks();
	int32_t r = 0;
	if (sub_blocks->get_num_elements() == 0)
...
CTaskTree::CTaskTree(std::shared_ptr<CTask> root_task) : CTaskRelation(),
	m_root_task(NULL)
{
...
SGVector<index_t>* CTaskTree::get_tasks_indices() const
{
	...
	auto iterator = blocks->get_first_element()->as<CTask>();
	do
	{
	...
	while ((iterator = blocks->get_next_element()->as<CTask>()) != NULL);
